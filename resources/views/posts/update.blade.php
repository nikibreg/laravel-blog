                    ...
                    {{ method_field('PATCH') }}
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                <br>
                                <input type="name" class="form-control" value="{{ $post->title }}"   placeholder="Title" name="title">
                            </div>
                            <div class="form-group">
                                <label class="text-gray-500" for="exampleInputEmail1">Post Text</label>
                                <br>
                                <input type="name" class="form-control" value="{{ $post->text }}" placeholder="Text" name="text">
                            </div>
                            <div class="form-group">
                                <label class="text-gray-500" for="exampleInputEmail1">Post Likes</label>
                                <br>
                                <input type="name" class="form-control" value="{{ $post->likes }}" placeholder="Likes" name="likes">
                            </div>
                        </div>
                        <select name="tags[]" id="tags" multiple>
                            @foreach ($tags as $tag)
                                <option {{ $post->tags->contains($tag) ? 'selected' : '' }} value="{{ $tag->name }}">
                                    {{$tag->name}}
                                </option>
                            @endforeach
                        </select>
